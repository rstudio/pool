...
- func:
  ...
  A function that has one argument, `conn` (a database connection
...
but its arguments work a little differently. First, it takes in a pool
...
This function (the `func` argument) gives you an argument to use in its
body, a database connection. So, you can use connection methods without
ever having to check out a connection. But you can also use arbitrary R
code inside the `func`'s body. This function will be called once we
fetch a connection from the pool. Once the function returns, we release
the connection back to the pool.

Like its DBI sister
[`DBI::dbWithTransaction()`](https://dbi.r-dbi.org/reference/dbWithTransaction.html),
this function calls `dbBegin()` before executing the code, and
`dbCommit()` after successful completion, or `dbRollback()` in case of
an error. This means that calling `poolWithTransaction` always has side
effects, namely to commit or roll back the code executed when `func` is
called. In addition, if you modify the local R environment from within
`func` (e.g. setting global variables, writing to disk), these changes
will persist after the function has returned.

Also, like
[`DBI::dbWithTransaction()`](https://dbi.r-dbi.org/reference/dbWithTransaction.html),
there is also a special function called
[`dbBreak()`](https://dbi.r-dbi.org/reference/dbWithTransaction.html)
that allows for an early, silent exit with rollback. It can be called
only from inside `poolWithTransaction`.
